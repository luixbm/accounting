...
<div class="card" style="max-width:660px">
  <form method="post" action="<?= $action ?>" enctype="multipart/form-data">
    <?= csrf_field() ?>
    <div class="row">
      <div class="field" style="max-width:160px"><label><?= lang('Report.col_code') ?></label><input name="code" class="mono" value="<?= esc($v('code')) ?>" required></div>
      <div class="field"><label><?= lang('Setup.display_name') ?></label><input name="name" value="<?= esc($v('name')) ?>" required></div>
    </div>
    <div class="field"><label><?= lang('Setup.legal_name') ?></label><input name="legal_name" value="<?= esc($v('legal_name')) ?>"></div>
    <div class="row">
      <div class="field" style="max-width:260px"><label>NPWP</label><input name="npwp" class="mono" value="<?= esc($v('npwp')) ?>"></div>
      <div class="field" style="max-width:200px">
        <label><?= lang('Setup.base_currency') ?></label>
        <?php $bc = $v('base_currency', 'IDR'); ?>
        <select name="base_currency" <?= $company ? 'disabled title="' . esc(lang('Setup.base_currency_fixed'), 'attr') . '"' : '' ?>>
    ...
    <div class="field">
      ...
      <?php if ($logo): ?>
        <div class="inline" style="margin-bottom:8px">
          ...
          <label class="inline" style="font-weight:400"><input type="checkbox" name="remove_logo" value="1" style="width:auto"> <?= lang('Setup.remove') ?></label>
        </div>
      <?php endif ?>
    ...
    <label class="inline" style="font-weight:400">
      <input type="checkbox" name="is_active" value="1" style="width:auto" <?= old('is_active', $company['is_active'] ?? 1) ? 'checked' : '' ?>> <?= lang('App.active') ?>
    </label>

    <div class="btn-group" style="margin-top:14px">
      <button class="btn" type="submit"><?= lang('App.save') ?></button>
      <a class="btn ghost" href="<?= site_url('companies') ?>"><?= lang('App.cancel') ?></a>
    </div>
  </form>
</div>
