...
<div class="page-head"><h1>Pay Supplier</h1></div>
...
<?php if ($supplierId && ! $open): ?>
  <div class="card"><p class="muted">No open invoices for this supplier.</p></div>
<?php elseif ($supplierId): ?>
  <form method="post" action="<?= site_url('purchases/payments') ?>" id="payform">
    ...
    <div class="card">
      <div class="row">
        <div class="field" style="max-width:160px"><label>Payment date</label><input type="date" name="payment_date" value="<?= date('Y-m-d') ?>" required></div>
      ...
      <p class="muted small">A payment settles one currency at a time. A/P clears at each invoice's rate; the rate difference posts to realized FX.</p>
    </div>

    <div class="card">
      <h2>Open invoices</h2>
      <table class="grid tight">
        <thead><tr><th>No.</th><th>Date</th><th>Ref</th><th>Cur</th><th class="right">Outstanding</th><th class="right">Pay this</th></tr></thead>
        <tbody>
        ...
        <tfoot>
          <tr class="subtotal"><td colspan="5" class="right">Total payment <span id="totCcy" class="muted"></span></td><td class="right mono" id="payTot">0.00</td></tr>
        </tfoot>
      </table>
      <div class="btn-group" style="margin-top:10px">
        <button type="button" class="btn sm ghost" id="fillAll">Pay all in full</button>
        <button type="button" class="btn sm ghost" id="clearAll">Clear</button>
      </div>
    </div>

    <div class="card">
      <button class="btn" type="submit">Record &amp; post payment</button>
