...
<div class="page-head">
  <div><h1><?= lang('Import.pp_h') ?></h1><div class="muted small"><?= lang('Import.pp_note') ?></div></div>
  <div class="btn-group">
    <a class="btn ghost" href="<?= site_url('purchases/payments/import/template') ?>"><?= lang('Import.download_template') ?></a>
    <a class="btn ghost" href="<?= site_url('purchases/payments') ?>"><?= lang('Import.pp_back') ?></a>
  </div>
</div>
...
<div class="card" style="max-width:680px">
  ...
  <form method="post" action="<?= site_url('purchases/payments/import') ?>" enctype="multipart/form-data">
    <?= csrf_field() ?>
    <div class="field">
      <label><?= lang('Import.pp_file') ?></label>
      <input type="file" name="file" accept=".xlsx,.xls,.csv" required>
    </div>
    <button class="btn" type="submit"><?= lang('Import.upload_continue') ?></button>
  </form>
  <p class="muted small" style="margin-top:10px">
    <?= lang('Import.pp_upload_note', [site_url('purchases/payments/import/template')]) ?>
  </p>
</div>
...
<div class="card">
  ...
  <table class="grid tight">
    <thead><tr><th>#</th><th><?= lang('Import.col_file') ?></th><th><?= lang('App.status') ?></th><th class="right"><?= lang('Import.pp_col_payments') ?></th><th class="right"><?= lang('Import.pp_col_unmatched') ?></th><th><?= lang('Import.col_when') ?></th><th></th></tr></thead>
    <tbody>
          ...
          <td class="right nowrap">
            <?php if ($committed): ?>
              <form method="post" action="<?= site_url('purchases/payments/import/' . $b['id'] . '/revert') ?>" style="display:inline"
                onsubmit="return confirm('<?= esc(lang('Import.pp_revert_confirm'), 'js') ?>')">
                <?= csrf_field() ?><button class="btn sm danger" type="submit"><?= lang('Import.revert') ?></button>
              </form>
            <?php else: ?>
              <a class="btn sm ghost" href="<?= site_url('purchases/payments/import/' . $b['id'] . '/map') ?>"><?= lang('Import.resume') ?></a>
            <?php endif ?>
          </td>
        </tr>
      <?php endforeach ?>
