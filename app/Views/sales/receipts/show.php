...
<div class="page-head">
  <div>
    <h1><?= esc($pay['receipt_no']) ?> <?= status_badge($pay['status'] === 'void' ? 'void' : 'posted') ?>
      <?php if ($isDeposit): ?><span class="badge badge-gray">down payment</span><?php endif ?></h1>
    <div class="muted small">
      <?= esc($pay['customer_name']) ?> · <?= date_id($pay['receipt_date']) ?> · <?= $isDeposit ? 'into' : 'from' ?> <?= esc($pay['bank_name']) ?>
  ...
  <div class="btn-group no-print">
    <?php if ($isDeposit && $pay['status'] === 'posted' && (float) $pay['unapplied'] > 0.005 && user_can('journal.post')): ?>
      <a class="btn" href="<?= site_url('sales/receipts/' . $pay['id'] . '/apply') ?>">Apply to invoices</a>
    <?php endif ?>
    <?php if ($pay['journal_id']): ?><a class="btn ghost" href="<?= site_url('journals/' . $pay['journal_id']) ?>">Journal</a><?php endif ?>
    <button class="btn secondary" onclick="window.print()">Print</button>
    <a class="btn ghost" href="<?= site_url('sales/receipts') ?>">Back</a>
  </div>
</div>
              ...
              <tr>
                ...
                <td class="right no-print">
                  <?php if ($pay['status'] === 'posted' && user_can('journal.void')): ?>
                    <form method="post" action="<?= site_url('sales/receipts/' . $pay['id'] . '/unapply') ?>" onsubmit="return confirm('Reverse this allocation? The deposit balance and the invoices are restored.')">
...
<?php else: ?>
  <div class="card" style="max-width:560px">
    <table class="grid tight">
      <thead><tr><th>Invoice</th><th>Customer ref</th><th class="right">Applied (<?= esc($cc) ?>)</th><?php if ($foreign): ?><th class="right">at inv. rate (<?= base_code() ?>)</th><?php endif ?></tr></thead>
      <tbody>
      ...
      <tfoot>
        <tr><td colspan="2" class="right">Cash received</td><td class="right mono"><?= money($pay['amount']) ?></td><?php if ($foreign): ?><td class="right mono"><?= money($pay['amount_base']) ?></td><?php endif ?></tr>
...
<?php if ($pay['status'] === 'posted' && user_can('journal.void')): ?>
  <div class="card">
    <h2>Void this <?= $isDeposit ? 'deposit' : 'receipt' ?></h2>
    <form method="post" action="<?= site_url('sales/receipts/' . $pay['id'] . '/void') ?>" class="inline"
      onsubmit="return confirm('Void this <?= $isDeposit ? 'deposit' : 'receipt' ?>? <?= $isDeposit ? 'It must have no applications.' : 'The invoices it paid will be reopened.' ?>')">
      <?= csrf_field() ?>
