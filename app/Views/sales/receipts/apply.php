...
<div class="page-head">
  <div>
    <h1><?= lang('Txn.apply_h', [esc($dep['receipt_no'])]) ?></h1>
  ...
  <div class="btn-group"><a class="btn ghost" href="<?= site_url('sales/receipts/' . $dep['id']) ?>"><?= lang('App.back') ?></a></div>
</div>

<?php if (! $open): ?>
  <div class="card"><p class="muted"><?= lang('Txn.no_open_inv_dp_ccy') ?></p></div>
<?php else: ?>
  <form method="post" action="<?= site_url('sales/receipts/' . $dep['id'] . '/apply') ?>" id="af">
    ...
    <div class="card">
      ...
      <div class="btn-group" style="margin-top:10px">
        <button type="button" class="btn sm ghost" id="fill"><?= lang('Txn.use_full_dp') ?></button>
    ...
    <div class="card">
      ...
      <a class="btn ghost" href="<?= site_url('sales/receipts/' . $dep['id']) ?>"><?= lang('App.cancel') ?></a>
    </div>
  </form>
