...
<div class="page-head">
  <div><h1><?= lang('Import.jb_h') ?></h1><div class="muted small"><?= lang('Import.jb_note') ?></div></div>
  <div class="btn-group"><a class="btn ghost" href="<?= site_url('jobs') ?>"><?= lang('Import.jb_back') ?></a></div>
</div>
...
<div class="card">
  ...
  <table class="grid tight">
    <thead><tr><th>#</th><th><?= lang('Import.col_file') ?></th><th><?= lang('App.status') ?></th><th class="right"><?= lang('Import.jb_col_jobs') ?></th><th class="right"><?= lang('Import.jb_col_skipped') ?></th><th><?= lang('Import.col_when') ?></th><th></th></tr></thead>
    <tbody>
          ...
          <td class="right nowrap">
            <?php if ($committed): ?>
              <form method="post" action="<?= site_url('jobs/import/' . $b['id'] . '/revert') ?>" style="display:inline"
                onsubmit="return confirm('<?= esc(lang('Import.jb_revert_confirm'), 'js') ?>')">
                <?= csrf_field() ?><button class="btn sm danger" type="submit"><?= lang('Import.revert') ?></button>
              </form>
            <?php else: ?>
              <a class="btn sm ghost" href="<?= site_url('jobs/import/' . $b['id'] . '/map') ?>"><?= lang('Import.resume') ?></a>
            <?php endif ?>
          </td>
        </tr>
      <?php endforeach ?>
